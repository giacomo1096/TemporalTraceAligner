...
public class Utilities {
	...
	// temporal
	public static void findActivitiesDuration(Trace trace){
		Vector<Long> timestamp_vector = trace.getTimestamp_vector();
		Vector<Long> action_durations_vector = new Vector<Long>();

		if(timestamp_vector.size()>0){
			long previous = (long) timestamp_vector.elementAt(0) - 1000;
			for(int i=0; i<timestamp_vector.size(); i++){
				long elem1 = (long) timestamp_vector.elementAt(i);
				long duration = elem1 - previous;
				previous = elem1;
				action_durations_vector.add(duration);
			}
			trace.setAction_durations_vector(action_durations_vector);
		}
	}
	...
	public static StringBuffer createPropositionalDomain(Trace trace) {
		...
		//if(Constants.getPlannerPerspective().getCostCheckBox().isSelected()) { 
			...
			PDDL_domain_buffer.append("(total-duration)\n");	
			PDDL_domain_buffer.append("(curr_timestamp)\n\n");	
			
			String label = new String();
			for(int i=0; i<trace.getTraceAlphabet_vector().size(); i++){
				label = trace.getTraceAlphabet_vector().elementAt(i);
				PDDL_domain_buffer.append("(dur_min_"+label+")\n");	
				PDDL_domain_buffer.append("(dur_max_"+label+")\n");	
			}

			PDDL_domain_buffer.append("\n");
			PDDL_domain_buffer.append("(dur ?a - activity)\n");	
			PDDL_domain_buffer.append("(timestamp ?a - activity)\n");
		...
		for(int i=0;i<Constants.getCombinationOfRelevantTransitions_vector().size();i++) {
			...
			if(trace.getTraceAlphabet_vector().contains(label_of_the_cot) || Constants.getAlphabetOfTheConstraints_vector().contains(label_of_the_cot)) {
				...
				PDDL_domain_buffer.append(":parameters ()\n");
				...
				for(int j=0; j<trace.getTrace_content_vector_timestamped().size(); j++){

					String activity = trace.getTrace_content_vector_timestamped().elementAt(j);


					PDDL_domain_buffer.append("(when (> (timestamp "+activity+") (curr_timestamp)) (increase (timestamp "+activity+ ") (dur_min_"+label_of_the_cot+")))\n");

				}

				PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_min_"+label_of_the_cot+"))\n");
				PDDL_domain_buffer.append("(increase (total-duration) (dur_min_"+label_of_the_cot+"))\n");
					...
					PDDL_domain_buffer.append("(increase (total-cost) ");	
						...
						for(int yu=0;yu<Constants.getActivitiesCost_vector().size();yu++) {
							...
							if(specificTraceCostVector.elementAt(0).equalsIgnoreCase(label_of_the_cot)) {
								int fixed_cost = Integer.parseInt(specificTraceCostVector.elementAt(1)) + 1;
				...
				//System.out.println(PDDL_domain_buffer.toString()); 
			...
			if(trace.getTraceAlphabet_vector().contains(label_of_the_cot) || Constants.getAlphabetOfTheConstraints_vector().contains(label_of_the_cot)) {
				...
				for(int k=0;k<trace.getOriginalTraceContent_vector().size();k++) {
					...
					if(trace.getOriginalTraceContent_vector().elementAt(k).equalsIgnoreCase(label_of_the_cot)) {
						...
						String cont1 = "n" + String.valueOf(cont);
					

						String activity = trace.getTrace_content_vector_timestamped().elementAt(k);

						//SYNC
						PDDL_domain_buffer.append("(:action sync" + "-" + label_of_the_cot + "-" + cot.getId() + "-" + cont1 + "\n"); //TODO aggiungere label _cont alla fine (anche nelle azioni seguenti) 
						PDDL_domain_buffer.append(":parameters ()\n");
						PDDL_domain_buffer.append(":precondition (and ");
						PDDL_domain_buffer.append("(currstate t" + k + ") ");
						PDDL_domain_buffer.append(cot.getPDDL_preconditions() + "\n");
						PDDL_domain_buffer.append("(>= (dur "+activity+") (dur_min_"+label_of_the_cot+")) (<= (dur "+activity+") (dur_max_"+label_of_the_cot+")))\n");

						PDDL_domain_buffer.append(":effect (and ");
						int j = k+1;
						PDDL_domain_buffer.append("(not (currstate t" + k + ")) " + "(currstate t" + j + ") " );
						PDDL_domain_buffer.append(cot.getPDDL_effects() + "\n");
						PDDL_domain_buffer.append("(increase (curr_timestamp) (dur "+activity+"))\n");
						PDDL_domain_buffer.append("(increase (total-duration) (dur "+activity+")))\n");
						PDDL_domain_buffer.append(")\n\n");

						//EXPANDED SYNK MIN
						PDDL_domain_buffer.append("(:action expanded-sync-min" + "-" + label_of_the_cot + "-" + cot.getId() + "-" + cont1 + "\n");
						PDDL_domain_buffer.append(":parameters ()\n");
						PDDL_domain_buffer.append(":precondition (and ");
						PDDL_domain_buffer.append("(currstate t" + k + ") ");
						PDDL_domain_buffer.append(cot.getPDDL_preconditions() + "\n");
						PDDL_domain_buffer.append("(< (dur "+activity+") (dur_min_"+label_of_the_cot+")))\n");

						PDDL_domain_buffer.append(":effect (and ");
						PDDL_domain_buffer.append("(not (currstate t" + k + ")) " + "(currstate t" + j + ") " );
						PDDL_domain_buffer.append(cot.getPDDL_effects() + "\n");
						
						String time_effects = new String();
						String next_activity = new String();
						for(int z=k; z<trace.getOriginalTraceContent_vector().size(); z++){
							next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
							time_effects += "(increase (timestamp "+next_activity+") (- (dur_min_"+label_of_the_cot+") (dur "+activity+")))\n";						
						}
						PDDL_domain_buffer.append(time_effects);
						PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_min_"+label_of_the_cot+"))\n");
						PDDL_domain_buffer.append("(increase (total-duration) (dur_min_"+label_of_the_cot+"))\n");
						...
						//EXPANDED SYNK MAX
						PDDL_domain_buffer.append("(:action expanded-sync-max" + "-" + label_of_the_cot + "-" + cot.getId() + "-" + cont1 + "\n");
						PDDL_domain_buffer.append(":parameters ()\n");
						PDDL_domain_buffer.append(":precondition (and ");
						PDDL_domain_buffer.append("(currstate t" + k + ") ");
						PDDL_domain_buffer.append(cot.getPDDL_preconditions() + "\n");
						...
						time_effects = new String();
						next_activity = new String();
						for(int z=k; z<trace.getOriginalTraceContent_vector().size(); z++){
							next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
							time_effects += "(increase (timestamp "+next_activity+") (- (dur_max_"+label_of_the_cot+") (dur "+activity+")))\n";						
						}
						PDDL_domain_buffer.append(time_effects);
						PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_max_"+label_of_the_cot+"))\n");
						PDDL_domain_buffer.append("(increase (total-duration) (dur_max_"+label_of_the_cot+"))\n");
						PDDL_domain_buffer.append("(increase (total-cost) 1))\n");
						PDDL_domain_buffer.append(")\n\n");

						//SHRINKED SYNK MAX
						PDDL_domain_buffer.append("(:action shrinked-sync-max" + "-" + label_of_the_cot + "-" + cot.getId() + "-" + cont1 + "\n");
						PDDL_domain_buffer.append(":parameters ()\n");
						PDDL_domain_buffer.append(":precondition (and ");
						PDDL_domain_buffer.append("(currstate t" + k + ") ");
						PDDL_domain_buffer.append(cot.getPDDL_preconditions() + "\n");
						PDDL_domain_buffer.append("(> (dur "+activity+") (dur_max_"+label_of_the_cot+")))\n");
						...
						for(int z=k; z<trace.getOriginalTraceContent_vector().size(); z++){
							next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
							time_effects += "(decrease (timestamp "+next_activity+") (- (dur "+activity+") (dur_max_"+label_of_the_cot+")))\n";						
						}
						PDDL_domain_buffer.append(time_effects);
						PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_max_"+label_of_the_cot+"))\n");
						PDDL_domain_buffer.append("(increase (total-duration) (dur_max_"+label_of_the_cot+"))\n");
						...
						//SHRINKED SYNK MIN
						PDDL_domain_buffer.append("(:action shrinked-sync-min" + "-" + label_of_the_cot + "-" + cot.getId() + "-" + cont1 + "\n");
						PDDL_domain_buffer.append(":parameters ()\n");
						PDDL_domain_buffer.append(":precondition (and ");
						PDDL_domain_buffer.append("(currstate t" + k + ") ");
						PDDL_domain_buffer.append(cot.getPDDL_preconditions() + "\n");
						...
						for(int z=k; z<trace.getOriginalTraceContent_vector().size(); z++){
							next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
							time_effects += "(decrease (timestamp "+next_activity+") (- (dur "+activity+") (dur_min_"+label_of_the_cot+")))\n";						
						}
						PDDL_domain_buffer.append(time_effects);
						PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_min_"+label_of_the_cot+"))\n");
						PDDL_domain_buffer.append("(increase (total-duration) (dur_min_"+label_of_the_cot+"))\n");
		...
		for(int gk=0;gk<trace.getOriginalTraceContent_vector().size();gk++) {
			...
			String activity = trace.getTrace_content_vector_timestamped().elementAt(gk);
			...
			PDDL_domain_buffer.append(":parameters ()\n");
			PDDL_domain_buffer.append(":precondition ");
			PDDL_domain_buffer.append("(and (currstate t" + gk + ") ");
			if(values_array.length>0) PDDL_domain_buffer.append(preconditionsSB + "\n");
			PDDL_domain_buffer.append("(>= (dur "+activity+") (dur_min_"+act+")) (<= (dur "+activity+") (dur_max_"+act+")))\n");
			...
			//EXPANDED SYNC MIN
			PDDL_domain_buffer.append("(:action expanded-sync-min" + "-" + act + "-t" + gk + "t" + j + "\n");
			PDDL_domain_buffer.append(":parameters ()\n");
			PDDL_domain_buffer.append(":precondition (and ");
			PDDL_domain_buffer.append("(currstate t" + gk + ") ");
			if(values_array.length>0) PDDL_domain_buffer.append(preconditionsSB + "\n");
			PDDL_domain_buffer.append("(< (dur "+activity+") (dur_min_"+ act +")))\n");
			...
			String time_effects = new String();
			String next_activity = new String();
			for(int z=gk; z<trace.getOriginalTraceContent_vector().size(); z++){
				next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
				time_effects += "(increase (timestamp "+next_activity+") (- (dur_min_"+act+") (dur "+activity+")))\n";						
			}
			PDDL_domain_buffer.append(time_effects);
			PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_min_"+act+"))\n");
			PDDL_domain_buffer.append("(increase (total-duration) (dur_min_"+act+"))\n");
			PDDL_domain_buffer.append("(increase (total-cost) 1))\n");
			PDDL_domain_buffer.append(")\n\n");

			//EXPANDED SYNC MAX
			PDDL_domain_buffer.append("(:action expanded-sync-max" + "-" + act + "-t" + gk + "t" + j + "\n");
			PDDL_domain_buffer.append(":parameters ()\n");
			PDDL_domain_buffer.append(":precondition (and ");
			PDDL_domain_buffer.append("(currstate t" + gk + ") ");
			if(values_array.length>0) PDDL_domain_buffer.append(preconditionsSB + "\n");
			...
			PDDL_domain_buffer.append(time_effects);
			PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_max_"+act+"))\n");
			PDDL_domain_buffer.append("(increase (total-duration) (dur_max_"+act+"))\n");
			PDDL_domain_buffer.append("(increase (total-cost) 1))\n");
			PDDL_domain_buffer.append(")\n\n");

			//SHRINKED SYNK MAX
			PDDL_domain_buffer.append("(:action shrinked-sync-max" + "-" + act + "-t" + gk + "t" + j + "\n");
			PDDL_domain_buffer.append(":parameters ()\n");
			PDDL_domain_buffer.append(":precondition (and ");
			PDDL_domain_buffer.append("(currstate t" + gk + ") ");
			if(values_array.length>0) PDDL_domain_buffer.append(preconditionsSB + "\n");
			PDDL_domain_buffer.append("(> (dur "+activity+") (dur_max_"+ act +")))\n");
			...
			for(int z=gk; z<trace.getOriginalTraceContent_vector().size(); z++){
				next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
				time_effects += "(decrease (timestamp "+next_activity+") (- (dur "+activity+") (dur_max_"+ act +")))\n";						
			}
			PDDL_domain_buffer.append(time_effects);
			PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_max_"+ act +"))\n");
			PDDL_domain_buffer.append("(increase (total-duration) (dur_max_"+ act +"))\n");
			...
			//SHRINKED SYNK MIN
			PDDL_domain_buffer.append("(:action shrinked-sync-min" + "-" + act + "-t" + gk + "t" + j + "\n");
			PDDL_domain_buffer.append(":parameters ()\n");
			PDDL_domain_buffer.append(":precondition (and ");
			PDDL_domain_buffer.append("(currstate t" + gk + ") ");
			if(values_array.length>0) PDDL_domain_buffer.append(preconditionsSB + "\n");
			...
			for(int z=gk; z<trace.getOriginalTraceContent_vector().size(); z++){
				next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
				time_effects += "(decrease (timestamp "+next_activity+") (- (dur "+activity+") (dur_min_"+ act +")))\n";						
			}
			PDDL_domain_buffer.append(time_effects);
			PDDL_domain_buffer.append("(increase (curr_timestamp) (dur_min_"+ act +"))\n");
			PDDL_domain_buffer.append("(increase (total-duration) (dur_min_"+ act +"))\n");
			...
			if(trace.getTraceAlphabet_vector().contains(label_of_the_cot) || Constants.getAlphabetOfTheConstraints_vector().contains(label_of_the_cot)) {
				...
				for(int k=0;k<trace.getOriginalTraceContent_vector().size();k++) {
					...
					if(trace.getOriginalTraceContent_vector().elementAt(k).equalsIgnoreCase(label_of_the_cot)) {
						...
						String time_effects = new String();
						String next_activity = new String();
						for(int z=k+1; z<trace.getOriginalTraceContent_vector().size(); z++){
							next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
							time_effects += "(decrease (timestamp "+next_activity+") (dur "+activity+"))\n";						
						}
						PDDL_domain_buffer.append(time_effects);
						...
						//if(Constants.getPlannerPerspective().getCostCheckBox().isSelected()) {
							...
							for(int yu=0;yu<Constants.getActivitiesCost_vector().size();yu++) {
								Vector<String> specificTraceCostVector = Constants.getActivitiesCost_vector().elementAt(yu);	
								if(specificTraceCostVector.elementAt(0).equalsIgnoreCase(label_of_the_cot)) {
									int fixed_cost = Integer.parseInt(specificTraceCostVector.elementAt(2)) + 1;
									int variable_cost = cot.getCost();
									int total_cost = fixed_cost + variable_cost;
									PDDL_domain_buffer.append(String.valueOf(total_cost) + "))\n");
									break;
		...
		for(int gk=0;gk<trace.getOriginalTraceContent_vector().size();gk++) {
			...
			String time_effects = new String();
			String next_activity = new String();
			for(int z=gk+1; z<trace.getOriginalTraceContent_vector().size(); z++){
				next_activity = trace.getTrace_content_vector_timestamped().elementAt(z);
				time_effects += "(decrease (timestamp "+next_activity+") (dur "+activity+"))\n";						
			}
			PDDL_domain_buffer.append(time_effects);
	...
	public static StringBuffer createPropositionalProblem(Trace trace) {
		...
		for(int i=0; i<trace.getTrace_content_vector_timestamped().size(); i++){
			activity = trace.getTrace_content_vector_timestamped().elementAt(i);
			PDDL_objects_buffer.append(activity + " - activity\n");
		}
		...
		PDDL_init_buffer.append("(= (total-duration) 0)\n"); 
		PDDL_init_buffer.append("(= (curr_timestamp) 0)\n");
		PDDL_init_buffer.append("\n"); 

		String durata = new String();
		String timestamp = new String();
		String act = new String();
		//TODO mettere vettore alfabeto non traccia getTraceAlphabet_vector
		for(int i=0; i<trace.getTraceAlphabet_vector().size(); i++){
			act = trace.getTraceAlphabet_vector().elementAt(i);
			PDDL_init_buffer.append("(= (dur_min_"+act+") 20000000 )\n");
			PDDL_init_buffer.append("(= (dur_max_"+act+") 70000000 )\n\n");
		}
		/* 
		for(int i=0; i<trace.getOriginalTraceContent_vector().size(); i++){
			act = trace.getOriginalTraceContent_vector().elementAt(i);
			PDDL_init_buffer.append("(= (dur_min_"+act+") 20000000 )\n");
			PDDL_init_buffer.append("(= (dur_max_"+act+") 70000000 )\n\n");
		}
		*/
		for(int i=0; i<trace.getTrace_content_vector_timestamped().size(); i++){
			activity = trace.getTrace_content_vector_timestamped().elementAt(i);
			durata = trace.getAction_durations_vector().elementAt(i).toString();
			timestamp = trace.getTimestamp_vector().elementAt(i).toString();
			PDDL_init_buffer.append("(= (dur "+activity+") "+durata+" )\n");
			PDDL_init_buffer.append("(= (timestamp "+activity+") "+timestamp+" )\n\n");
